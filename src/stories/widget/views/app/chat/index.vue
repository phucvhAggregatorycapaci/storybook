    ...
    >
        <div class="flex-none min-w-[260px]" v-if="window.width > 1024">
            <Card bodyClass=" relative p-0 h-full overflow-hidden ">
                <div
                    class="border-b border-slate-100 dark:border-slate-700 pb-4"
                >
                    <Myprofile />
                </div>
                <div
                    class="border-b border-slate-100 dark:border-slate-700 py-1"
                >
                    <div
        ...
        <div class="flex-1">
            <div class="parent flex space-x-5 h-full rtl:space-x-reverse">
                <div
                    class=""
                    :class="
                        this.openinfo && this.activechat === true
                            ? 'flex-1'
                            : 'flex-1'
                    "
                >
...
export default {
    ...
    methods: {
        enterMobileSidebar(el) {
            gsap.from(el, {
                x: this.$store.state.themeSettingsStore.direction ? 100 : -100,
                duration: 0.3,
                ease: "power3.out",
            });
        },
