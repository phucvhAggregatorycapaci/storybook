    ...
    <header :class="navbarTypeClass()">
        <div
            :class="`app-header md:px-6 px-[15px]  dark:bg-slate-800 shadow-base dark:shadow-base3 bg-white ${borderSwicthClass()} ${
                this.$store.state.themeSettingsStore.navbarColor
            }
      ${
          this.$store.state.themeSettingsStore.menuLayout === 'horizontal' &&
          window.width > 1280
              ? 'py-1'
              : 'md:py-6 py-3'
      }
                    ...
                    <button
                        ...
                        v-if="
                            this.$store.state.themeSettingsStore
                                .sidebarCollasp && window.width > 1280
                        "
                        @click="
                            this.$store.state.themeSettingsStore.sidebarCollasp = false
                        "
                    >
                        <Icon
                            icon="akar-icons:arrow-right"
                            v-if="
                                !this.$store.state.themeSettingsStore.direction
                            "
                        />
                        <Icon
...
export default {
    ...
    methods: {
        navbarTypeClass() {
            switch (this.$store.state.themeSettingsStore.navbarType) {
                case "floating":
                    return "floating";
                case "sticky":
        ...
        borderSwicthClass() {
            if (
                this.$store.state.themeSettingsStore.skin === "bordered" &&
                this.$store.state.themeSettingsStore.navbarType !== "floating"
            ) {
