<template>
  <ul>
    <li
      ...
      ${$route.name&&$route.name === item.link ? 'menu-item-active' : ''}
...
export default defineComponent({
  ...
  props: {
    ...
    items: { default:[] as any[], required: true },
  ...
  watch: {
    $route() {
      if (this.$store.state.themeSettings.mobielSidebar) {
        this.$store.state.themeSettings.mobielSidebar = false;
      }
